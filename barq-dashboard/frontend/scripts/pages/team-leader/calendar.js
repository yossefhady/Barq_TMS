...
async function populateUserFilter() {
    try {
        ...
        // Filter direct reports (Team Leader Logic)
        teamMembers = users.filter(u => {
            const uId = u.UserId || u.userId;
            const uTlId = u.TeamLeaderId || u.teamLeaderId;
            return uId != myId && uTlId == myId;
        });
        
        const select = document.getElementById("calendarUserFilter");
        if (select) {
            teamMembers.sort((a, b) => (a.Name || a.Username).localeCompare(b.Name || b.Username));
...
async function loadCalendarData() {
  try {
    ...
    // Safer range: 2 months before and 2 months after to cover all edge cases
    ...
    // Fetch events and tasks (Users already fetched in populateUserFilter)
    ...
    if (currentFilterUserId) {
        // Specific User View: Show only their tasks
        tasks = tasksResponse.filter(t => t.AssignedTo == currentFilterUserId);
    } else {
        // "My Calendar" View: Show My Tasks + Team Tasks
        const teamMemberIds = teamMembers.map((u) => u.UserId || u.userId);
        ...
        // Team tasks: Assigned to someone in my team list
        const teamTasks = tasksResponse.filter((t) =>
            teamMemberIds.some(id => id == t.AssignedTo)
        );
