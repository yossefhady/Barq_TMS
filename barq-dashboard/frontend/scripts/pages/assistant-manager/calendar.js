...
let allUsers = [];
let teamMembers = []; // Store team members for modal attendees logic
...
async function populateUserFilter() {
    try {
        ...
        team.forEach(u => {
            ...
            if (uid && uid !== myId && !seenIds.has(uid)) {
                seenIds.add(uid);
                uniqueTeam.push(u);
            }
        });
        
        teamMembers = uniqueTeam; // Assign to global for modal use
        
        const select = document.getElementById("calendarUserFilter");
        if (select) {
            uniqueTeam.sort((a, b) => (a.Name || a.Username).localeCompare(b.Name || b.Username));
            uniqueTeam.forEach(u => {
