...
async function loadCalendarData() {
  try {
    ...
    // My Tasks only (Usually Clients see tasks related to their Projects)
    // Here we filter by AssignedTo, which might be incorrect for Clients (tasks are assigned to Emps).
    // Clients should see tasks WHERE ClientId == Me (implied by User Role).
    // But for now, sticking to standard "AssignedTo" unless prompt specified otherwise.
    // Actually, Clients usually have Project related tasks.
    // If I filter AssignedTo, it will likely be empty.
    // However, I will stick to "My Tasks" pattern requested for "Employee" which was the template.
    // If Client acts as a User who has tasks assigned, this is correct.
