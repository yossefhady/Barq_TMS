...
let events = [];
let tasks = [];
 // No specific 'teamMembers' needed for Manager unless for other logic, but for filter we fetch all
let currentUser = null;
let currentFilterUserId = null;
let currentEditId = null;
let allUsers = []; // Store users for filter
let teamMembers = []; // Needed for modal attendees logic
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  setupEventListeners();
  await populateUserFilter(); // Load users first
...
async function populateUserFilter() {
    try {
        ...
        allUsers = users;
        teamMembers = users; // Assign for modal use
        const select = document.getElementById("calendarUserFilter");
        if (select) {
            // Sort users by name
            ...
            users.forEach(u => {
                ...
                option.value = u.UserId || u.userId;
                option.textContent = u.Name || u.Username;
                // Don't exclude self, allow selecting explicitly too
                if ((u.UserId || u.userId) != currentUser.UserId) {
                   select.appendChild(option);
                }
            });
        }
    } catch (e) {
        console.error("Error loading users for filter:", e);
    }
}
...
function handleUserFilterChange() {
    ...
    currentFilterUserId = val ? parseInt(val) : null;
    loadCalendarData();
}
...
async function loadCalendarData() {
  try {
    ...
    console.log("Fetching calendar data for range:", startRange.toISOString(), "to", endRange.toISOString());
    console.log("Filter Target User:", currentFilterUserId);
    ...
    if (currentFilterUserId) {
        calendarFilter.TargetUserId = currentFilterUserId;
    }

    // Fetch events and tasks
    const [eventsResponse, tasksResponse] = await Promise.all([
      API.Calendar.getEvents(calendarFilter).catch((err) => {
    ...
    console.log("Events fetched:", eventsResponse);

    events = eventsResponse.Events || eventsResponse.events || [];

    // Filter tasks based on selected user context
    // If viewing specific user -> show that user's tasks
    // If viewing "My Calendar" -> show My Tasks
    
    // Determine the effective user ID we are viewing
    // (If currentFilterUserId is null, we are viewing Ourselves)
    currentUser = auth.getCurrentUser();
    const viewingUserId = currentFilterUserId || (currentUser?.UserId || currentUser?.userId);

    const relevantTasks = tasksResponse.filter(
      (t) => t.AssignedTo == viewingUserId
    );

    // Note: For Manager view, we simplify "Team Tasks" logic. 
    // Usually "My Calendar" mode might want to show Team Tasks too?
    // Requirement says: "see eachone calender only or his cal".
    // "His cal" usually implies his tasks + team tasks he monitors?
    // For simplicity and strict compliance with "eachone or his", we show:
    // 1. If Target Selected: Only THAT user's tasks.
    // 2. If "My Calendar": My Tasks.
    // We will drop the complex "Team Leader" logic of merging team tasks for now to keep it clean, 
    // unless "My Calendar" for Manager implicitly contains everything. 
    // Given the filter "View As", it's better to isolate contexts.
